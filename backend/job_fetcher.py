...
def _fallback_jobs(skills: List[str], location: str, preferred_role: str = "") -> List[Dict]:
    normalized_skills = _unique_items(skills)
    roles = [preferred_role] if preferred_role and preferred_role.strip() else _infer_role_queries(normalized_skills)
    if len(roles) < 3:
        roles = _unique_items(roles + _infer_role_queries(normalized_skills) + ["Software Engineer"])

    jobs: List[Dict] = []
    for index in range(6):
        role = roles[index % len(roles)]
        required_skills = _build_required_skills(normalized_skills) or ["Python", "SQL", "Git", "REST API"]
        company = FALLBACK_COMPANIES[index % len(FALLBACK_COMPANIES)]
        jobs.append(
            {
                "title": role,
                "company": company,
                ...
                "description": (
                    f"{company} is looking for a {role} with hands-on experience in "
                    f"{', '.join(required_skills[:4])}. This local fallback result appears "
                    "when SerpAPI is not configured or returns no jobs."
                ),
...
def fetch_jobs(
    ...
    if not api_key:
        _debug_log("SERPAPI_API_KEY is missing; returning local fallback jobs.")
        return _fallback_jobs(skills, location, preferred_role)
    ...
    return jobs[:20] or _fallback_jobs(skills, location, preferred_role)
